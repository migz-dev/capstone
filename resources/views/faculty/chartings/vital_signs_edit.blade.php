...
<head>
  ...
  <title>Edit Vital Signs · NurSync — CI</title>
...
<main class="min-h-screen flex">
  ...
  <section class="flex-1 px-6 md:px-8 py-8 md:py-10">
    @php
      ...
      $takenHdr  = optional($vital->taken_at)->format('Y-m-d H:i') ?? '—';
    @endphp
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold">Edit Vital Signs</h1>
          <p class="text-[13px] text-slate-500 mt-0.5">Taken {{ $takenHdr }}</p>
        </div>
      </div>
    ...
    {{-- Form --}}
    <form action="{{ route('faculty.chartings.vital_signs.update', $vital) }}" method="POST" class="mt-6" novalidate>
      @csrf @method('PUT')
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-5">
        <div class="grid md:grid-cols-3 gap-4">
          {{-- Taken at --}}
          <div>
            <label for="taken_at" class="block text-[12px] text-slate-500 mb-1">Taken at <span class="text-rose-600">*</span></label>
            <input id="taken_at" type="datetime-local" name="taken_at"
                   value="{{ old('taken_at', $vital->taken_at?->format('Y-m-d\TH:i')) }}"
                   class="w-full rounded-xl border-slate-300 text-[14px] @error('taken_at') ring-2 ring-rose-300 @enderror" required />
        ...
        <div class="grid md:grid-cols-6 gap-3">
          <div>
            ...
            <input id="temp_c" type="number" step="0.1" min="30" max="45" name="temp_c"
                   value="{{ old('temp_c', $vital->temp_c) }}"
                   placeholder="T °C"
                   class="w-full rounded-xl border-slate-300 text-[14px] @error('temp_c') ring-2 ring-rose-300 @enderror" />
          ...
          <div>
            ...
            <input id="pulse_bpm" type="number" min="20" max="220" name="pulse_bpm"
                   value="{{ old('pulse_bpm', $vital->pulse_bpm) }}"
                   placeholder="P bpm"
                   class="w-full rounded-xl border-slate-300 text-[14px] @error('pulse_bpm') ring-2 ring-rose-300 @enderror" />
          ...
          <div>
            ...
            <input id="resp_rate" type="number" min="5" max="80" name="resp_rate"
                   value="{{ old('resp_rate', $vital->resp_rate) }}"
                   placeholder="R cpm"
                   class="w-full rounded-xl border-slate-300 text-[14px] @error('resp_rate') ring-2 ring-rose-300 @enderror" />
          ...
          <div>
            ...
            <input id="bp_systolic" type="number" min="50" max="260" name="bp_systolic"
                   value="{{ old('bp_systolic', $vital->bp_systolic) }}"
                   placeholder="BP Sys"
                   class="w-full rounded-xl border-slate-300 text-[14px] @error('bp_systolic') ring-2 ring-rose-300 @enderror" />
          ...
          <div>
            ...
            <input id="bp_diastolic" type="number" min="20" max="180" name="bp_diastolic"
                   value="{{ old('bp_diastolic', $vital->bp_diastolic) }}"
                   placeholder="BP Dia"
                   class="w-full rounded-xl border-slate-300 text-[14px] @error('bp_diastolic') ring-2 ring-rose-300 @enderror" />
          ...
          <div>
            ...
            <input id="spo2" type="number" min="0" max="100" name="spo2"
                   value="{{ old('spo2', $vital->spo2) }}"
                   placeholder="SpO₂ %"
                   class="w-full rounded-xl border-slate-300 text-[14px] @error('spo2') ring-2 ring-rose-300 @enderror" />
        ...
        <div class="grid md:grid-cols-3 gap-4 items-center">
          <div class="md:col-span-2">
            ...
            <input id="remarks" type="text" name="remarks"
                   value="{{ old('remarks', $vital->remarks) }}"
          ...
          <div>
            ...
            @php $oldPain = old('pain_scale', $vital->pain_scale ?? 0); @endphp
            <div class="flex items-center gap-3">
              <input id="pain_scale" type="range" min="0" max="10" name="pain_scale"
                     value="{{ $oldPain }}" class="w-full"
                     oninput="document.getElementById('painOut').textContent = this.value" />
      ...
      <div class="mt-5 flex items-center gap-2">
        <button type="submit" class="inline-flex items-center h-10 px-4 rounded-xl bg-emerald-600 text-white text-[13px] font-semibold">
          <i data-lucide="save" class="h-4 w-4 mr-2"></i> Update Vitals
        </button>
        <a href="{{ route('faculty.chartings.vital_signs.show', $vital) }}"
