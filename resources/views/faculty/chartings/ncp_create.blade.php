...
<head>
  ...
  <title>New Nursing Care Plan · NurSync — CI</title>
  ...
  <section class="flex-1 px-6 md:px-8 py-8 md:py-10">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold">New Nursing Care Plan</h1>
    ...
    <form class="mt-6" method="POST" action="{{ route('faculty.chartings.ncp.store') }}" novalidate>
      @csrf
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-6">
        <div class="grid md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="datetime-local" name="noted_at" value="{{ old('noted_at') }}"
        ...
        <div class="grid md:grid-cols-3 gap-4">
          <div>
            ...
            <textarea name="dx_primary" rows="3"
              class="w-full rounded-xl border-slate-300 text-[14px] @error('dx_primary') ring-2 ring-rose-300 @enderror"
              placeholder="Impaired gas exchange …">{{ old('dx_primary') }}</textarea>
          ...
          <div>
            ...
            <textarea name="dx_related_to" rows="3" class="w-full rounded-xl border-slate-300 text-[14px]">{{ old('dx_related_to') }}</textarea>
          </div>
          <div>
            <label class="block text-[12px] text-slate-500 mb-1">As evidenced by</label>
            <textarea name="dx_as_evidenced_by" rows="3" class="w-full rounded-xl border-slate-300 text-[14px]">{{ old('dx_as_evidenced_by') }}</textarea>
          </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <textarea name="goal_short" rows="3" class="w-full rounded-xl border-slate-300 text-[14px]">{{ old('goal_short') }}</textarea>
          </div>
          <div>
            <label class="block text-[12px] text-slate-500 mb-1">Long-term Goal</label>
            <textarea name="goal_long" rows="3" class="w-full rounded-xl border-slate-300 text-[14px]">{{ old('goal_long') }}</textarea>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea name="interventions" rows="6"
            class="w-full rounded-xl border-slate-300 text-[14px] @error('interventions') ring-2 ring-rose-300 @enderror"
            placeholder="1) Monitor vital signs …&#10;2) Encourage DBE …">{{ old('interventions') }}</textarea>
        ...
        <div>
          ...
          <textarea name="evaluation" rows="4" class="w-full rounded-xl border-slate-300 text-[14px]">{{ old('evaluation') }}</textarea>
        </div>
      ...
      <div class="mt-5 flex items-center gap-2">
        <button class="inline-flex items-center h-10 px-4 rounded-xl bg-emerald-600 text-white text-[13px] font-semibold">
          <i data-lucide="save" class="h-4 w-4 mr-2"></i> Save Plan
        </button>
        <a href="{{ route('faculty.chartings.ncp.index') }}" class="inline-flex items-center h-10 px-3 rounded-xl bg-slate-100 text-[13px]">Cancel</a>
      </div>
    </form>
  </section>
</main>
