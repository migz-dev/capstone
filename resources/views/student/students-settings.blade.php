        ...
        <div class="mt-6 rounded-2xl border border-slate-200/70 bg-white p-6">
          ...
          <form class="mt-5 space-y-6" method="POST" action="{{ route('student.settings.profile') }}" enctype="multipart/form-data">
            ...
            <div class="flex items-center gap-6">
              ...
              <div class="relative">
                @if($avatarUrl)
                  <img src="{{ $avatarUrl }}" alt="Profile photo" class="h-[84px] w-[84px] rounded-full object-cover border border-slate-200">
                @else
                  <div class="h-[84px] w-[84px] rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-700 font-semibold">
                    {{ $u->initials }}
                  </div>
                @endif
              </div>
